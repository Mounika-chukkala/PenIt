...
function CreateBlog() {
    ...
    // })
      const [BlogData, seBlogData] = useState({
        title: "",
        description: ""
      });
    
    let {token}=JSON.parse(localStorage.getItem(Blog"))
if(!token){
...
<div className="min-h-screen flex bg-[#F0FDF4] text-[#1E293B]">
      ...
      <div className="w-1/2 flex justify-center items-center p-10">
        <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
          <h2 className="text-2xl font-bold mb-6 text-[#10B981]">Create an account</h2>
          <input
            className="w-full mb-4 px-4 py-2 border border-[#6EE7B7] rounded focus:outline-none focus:ring-2 focus:ring-[#10B981]"
            type="text"
            placeholder="Name"
            onChange={(e) => seBlogData({ ..BlogData, name: e.target.value })}
          />
          <input
            className="w-full mb-4 px-4 py-2 border border-[#6EE7B7] rounded focus:outline-none focus:ring-2 focus:ring-[#10B981]"
            type="email"
            placeholder="Email"
            onChange={(e) => seBlogData({ ..BlogData, email: e.target.value })}
          />
          {/* <input
            className="w-full mb-6 px-4 py-2 border border-[#6EE7B7] rounded focus:outline-none focus:ring-2 focus:ring-[#10B981]"
            type="password"
            placeholder="Password"
            onChange={(e) => seBlogData({ ..BlogData, password: e.target.value })}
          /> */}
