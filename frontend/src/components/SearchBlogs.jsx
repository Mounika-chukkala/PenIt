...
<ChevronLeft size={30}  onClick={() => setPage((prev) => prev - 1)}
                  className={`rounded-full ${page==1 ? "text-slate-400": "text-blue-700"} font-extrabold text-3xl`}/>
                </button>    
                <button disabled={!hasMore}>

<ChevronRight size={30}  onClick={() => setPage((prev) => prev + 1)}
                  className={` ${!hasMore ? "text-slate-400": "text-blue-700"} font-extrabold  text-3xl`} />        
                  </button>
              </div>
          </div>
        ) : (
        ...
        <div className="space-y-2 flex flex-col items-end">
          {users.map((user) => (
            ...
            >
              <img src={`https://api.dicebear.com/9.x/initials/svg?seed=${user.username}` || user.name[0] } className="bg-blue-100 text-blue-700 w-10 h-10  flex items-center justify-center rounded-full font-bold"/>
